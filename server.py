...
@app.route('/')
def index():
    if 'total_gold' and 'activities' not in session:  #what does this mean and why
...
def process_gold():
    ...
    if gold_this_turn >= 0:
        ...
        new_message = f"<p class='text-danger'>Damn you lost! {gold_this_turn} gold from {location} {current_time}</p>"
        session['activities'] += new_message
